...
ALLOWED_IDS = [8317982282]  # ← Yangi ID qo'shish: [8317982282, 111111, 222222]
...
NAV_HTML = "<div class='nav'><div class='nav-brand'>🏪 SmartStore</div><div class='nav-links'><a href='/dashboard'>📊 Panel</a><a href='/pos'>🛒 Kassa</a><a href='/products'>📦 Mahsulot</a><a href='/sales'>🧾 Sotuv</a><a href='/debts'>💳 Qarzdor</a><a href='/reports'>📈 Hisobot</a><a href='/logout' style='color:var(--red);'>🚪 Chiqish</a></div></div>"
...
TG_SCRIPT = "<script src='https://telegram.org/js/telegram-web-app.js'></script><script>if(window.Telegram&&Telegram.WebApp){Telegram.WebApp.ready();Telegram.WebApp.expand();}(function(){var p=new URLSearchParams(location.search).get('tg_user');if(p&&p!=='0'){try{localStorage.setItem('ss_uid',p);}catch(e){}}else{try{var s=localStorage.getItem('ss_uid');if(s&&location.pathname!=='/login'&&location.pathname!=='/logout'){location.replace(location.pathname+'?tg_user='+s);}}catch(e){}}})();</script>"
    ...
    if request.method == "POST":
        ...
        if tg_id.isdigit():
            ...
            if uid not in ALLOWED_IDS:
                return render_template_string("<!DOCTYPE html><html><head><meta charset='utf-8'><title>Login</title><style>" + CSS + "</style></head><body><div style='padding:24px;max-width:500px;margin:60px auto;'><div class='card' style='text-align:center;padding:40px;'><div style='font-size:64px;margin-bottom:20px;'>🚫</div><h1 style='font-size:32px;margin-bottom:12px;color:var(--red);'>Kirish taqiqlangan!</h1><p style='color:var(--dim);margin-bottom:16px;'>Sizning ID: <strong style='color:#fff;'>" + str(uid) + "</strong></p><p style='color:var(--dim);margin-bottom:32px;'>Bu tizim faqat ruxsat berilgan foydalanuvchilar uchun.</p><a href='/login' class='btn btn-primary' style='padding:18px;'>← Orqaga</a></div></div></body></html>")
            session["tg_user"] = uid
            return redirect("/dashboard?tg_user=" + tg_id)
    return render_template_string("<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>SmartStore Login</title><style>" + CSS + "</style></head><body><div style='padding:24px;max-width:500px;margin:60px auto;'><div class='card' style='text-align:center;padding:40px;'><div style='font-size:64px;margin-bottom:20px;'>🔐</div><h1 style='font-size:32px;margin-bottom:12px;'>SmartStore</h1><p style='color:var(--dim);margin-bottom:32px;'>Telegram ID kiriting</p><form method='POST'><input class='input' name='tg_id' placeholder='Masalan: 8317982282' required style='font-size:20px;text-align:center;padding:20px;margin-bottom:16px;'><button class='btn btn-primary' style='width:100%;padding:18px;'>🚀 Kirish</button></form><p style='color:var(--dim);margin-top:24px;font-size:13px;'>💡 ID ni bilish: Telegram'da @userinfobot ga yozing</p></div></div></body></html>")
...
@app.route("/dashboard")
def dashboard():
    db = get_db(); uid = get_user_id(); today = datetime.now().strftime("%Y-%m-%d")
    ...
    return RP("<div style='padding:24px;max-width:1400px;margin:0 auto;'><h1 style='font-size:32px;margin-bottom:24px;'>📊 Dashboard</h1><div class='grid g4' style='margin-bottom:24px;'><div class='stat-card'><div class='stat-label'>💰 Bugungi savdo</div><div class='stat-value'>{{\"{:,.0f}\".format(ts)}}</div></div><div class='stat-card'><div class='stat-label'>🧾 Cheklar</div><div class='stat-value'>{{tc}}</div></div><div class='stat-card'><div class='stat-label'>📦 Mahsulotlar</div><div class='stat-value'>{{tp}}</div></div><div class='stat-card'><div class='stat-label'>⚠️ Kam qoldiq</div><div class='stat-value'>{{ls}}</div></div></div><div class='grid g2'><div class='card'><h2 style='margin-bottom:16px;'>🏆 Top 5</h2>{%for p in top%}<div style='display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid var(--border);'><span>{{p.name}}</span><span class='badge badge-blue'>{{p.t}} dona</span></div>{%else%}<p style='color:var(--dim);text-align:center;'>Hali savdo yo'q</p>{%endfor%}</div><div class='card'><h2 style='margin-bottom:16px;'>📈 7 kunlik</h2>{%for s in ws%}<div style='display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);'><span style='color:var(--dim);'>{{s.d}}</span><span style='color:var(--green);font-weight:700;'>{{\"{:,.0f}\".format(s.s)}}</span></div>{%endfor%}</div></div>{%if td>0%}<div class='card' style='margin-top:24px;'><div style='display:flex;justify-content:space-between;align-items:center;'><div><div style='color:var(--dim);'>💸 Jami qarz</div><div style='font-size:28px;font-weight:800;color:var(--yellow);'>{{\"{:,.0f}\".format(td)}} so'm</div></div><a href='/debts' class='btn btn-primary'>Qarzdorlar →</a></div></div>{%endif%}</div>", ts=ts, tc=tc, tp=tp, ls=ls, td=td, ws=ws, top=top)

@app.route("/products")
def products_list():
    db = get_db(); uid = get_user_id(); q = request.args.get("q", "")
...
def product_delete(pid):
    db = get_db(); db.execute("DELETE FROM products WHERE id=? AND user_id=?", (pid, get_user_id())); db.commit(); return redirect("/products")
...
def api_pbc():
    ...
    db = get_db(); p = db.execute("SELECT * FROM products WHERE barcode=? AND user_id=?", (c, get_user_id())).fetchone()
...
def api_checkout():
    try:
        ...
        db = get_db(); uid = get_user_id(); total = 0; prepared = []
        for item in items:
...
@app.route("/reports")
def reports_page():
    db = get_db(); uid = get_user_id(); p = request.args.get("period", "day")
    ...
    try:
        ...
        async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if user_id not in ALLOWED_IDS:
                await update.message.reply_html("🚫 <b>Kirish taqiqlangan!</b>\n\nSizning ID: <code>{}</code>\n\nBu bot faqat ruxsat berilgan foydalanuvchilar uchun.".format(user_id))
                return
...
if __name__ == "__main__":
    ...
    print("="*50); print("🏪 SmartStore POS (Web + Bot)"); print("="*50)
    print("🌐 Port:", os.environ.get("PORT", 5000)); print("="*50)
